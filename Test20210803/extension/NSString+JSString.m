...
/**
 处理后台时间格式得到2018-12-12类型
 */
+ (NSString*)getStandardTimeStrByServiceStr:(NSString*)timeStr{
    NSMutableString *resultStr = [[NSMutableString alloc]initWithString:timeStr];
    if(resultStr.length > 10){
        [resultStr deleteCharactersInRange:NSMakeRange(10, resultStr.length - 10)];
    }else if(timeStr == nil){
...
+ (NSString *)getTargetTimeStr:(NSString*)timeStr{
    NSMutableString *mutableTimeStr = [[NSMutableString alloc]initWithString:timeStr];
    NSString *resultStr = @"";
    if(mutableTimeStr.length){
        resultStr = [mutableTimeStr stringByReplacingOccurrencesOfString:@"-" withString:@"/"];
    }
    return resultStr;
}
...
{
    NSMutableString *orinalPriceStr = [NSMutableString stringWithString:priceStr];
    if (orinalPriceStr.length)
    {
        if ([orinalPriceStr containsString:@".00"])
        {
            NSArray *numArr = [orinalPriceStr componentsSeparatedByString:@"."];
            if (numArr.count > 1)
            {
                NSString *lastNumStr = numArr[1];
                NSInteger shouldDeleteLength = lastNumStr.length + 1;
                [orinalPriceStr replaceCharactersInRange:NSMakeRange(orinalPriceStr.length - shouldDeleteLength, shouldDeleteLength) withString:@""];
            }
        }else if ([orinalPriceStr containsString:@"."])
        {
            CGFloat orinalPrice = orinalPriceStr.floatValue;
            orinalPriceStr = [NSMutableString stringWithFormat:@"%.2lf",orinalPrice];
        }
    }
    return orinalPriceStr;
}
...
+ (CGFloat)calculateAttributeTitleHeight:(NSAttributedString*)courseNameAttStr width:(CGFloat)labelWidth textFont:(UIFont*)font
{
    ...
    [label setFixedWidth:[NSNumber numberWithFloat:labelWidth]];
    [label sizeToFit];
    return label.height;
}
...
+(NSString *)getClassNumStrWithTotalNum:(NSInteger)totalNum withEffectiveNum:(NSInteger)effectiveNum{
    if (totalNum == effectiveNum) {
        return [NSString stringWithFormat:@"共%ld",(long)effectiveNum];
    }else{
        return [NSString stringWithFormat:@"剩%ld",(long)effectiveNum];
    }
}
+(NSString *)getClassNumStrWithTotalNum:(NSString *)totalNum withEffectiveNum:(NSString *)effectiveNum withIsComplete:(Boolean)isComplete{
    if (isComplete) {
        return [NSString stringWithFormat:@"共%@",totalNum];
    }else{
        return [NSString stringWithFormat:@"剩%@",effectiveNum];
    }
}
...
-(NSString*)getPriceStr{
    if (self.isNumber) {
        ...
        if (priceStr.length > 1) {
               if ([priceStr componentsSeparatedByString:@"."].count == 2) {
                   NSString *last = [priceStr componentsSeparatedByString:@"."].lastObject;
                   if ([last isEqualToString:@"00"]) {
                       priceStr = [priceStr substringToIndex:priceStr.length - (last.length + 1)];
                       return priceStr;
                   }else{
